...
def init(env, player_relative, obs):
    ...
    for i in range(len(player_x)):
        ...
        if (len(unit_xy_list) >= 1):
            ...
            obs = env.step(
                actions=[sc2_actions.FunctionCall(_SELECT_CONTROL_GROUP, [[_CONTROL_GROUP_SET], [group_id]])])
            unit_xy_list = []
...
def select_marine(env, obs):
    ...
    if (danger_min_dist != None and danger_min_dist <= 5):
        ...
        while (len(group_list) > 0):
            ...
            obs = env.step(
                actions=[sc2_actions.FunctionCall(_SELECT_CONTROL_GROUP, [[_CONTROL_GROUP_RECALL], [group_id]])])
